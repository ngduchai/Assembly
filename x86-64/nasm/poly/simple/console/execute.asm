...
code_start:
	...
	; use LoadLibrary to load msvcrt.dll
	lea edx, [ebp+msvcrt]
	push edx
		call eax 	; After call, eax hold base address of msvcrt.dll load point
	
	; use GetProcAddress to load printf from mscvrt.dll which has jush been loaded
	lea edx, [ebp + printf]
	push edx
	push eax
	mov ebx, [ebp+addGetProcAddress]
		call ebx
	mov [ebp+add_printf], eax
		
	; printf hello
	mov ebx, [ebp + add_printf]
	lea eax, [ebp + msg]
	push eax
		call ebx
